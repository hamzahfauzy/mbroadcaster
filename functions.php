...
function sendMail($subject, $recipients, $message, $attachments = [])
{
    ...
    try {
        ...
        $mail->Host       = app('smtp_host');                     //Set the SMTP server to send through
        $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
        $mail->Username   = app('smtp_email');                     //SMTP username
        $mail->Password   = app('smtp_password');                               //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = app('smtp_port');                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
